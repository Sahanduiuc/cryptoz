...
def _chartdata(client, pair, from_dt, to_dt, period=None):
    ...
    if isinstance(period, Period):
        period = period.value
    chart_data = client.returnChartData(pair, period=period, start=dt_to_ts(from_dt), end=dt_to_ts(to_dt))
...
def chartdata(client, pairs, *args, **kwargs):
    if isinstance(pairs, str):
        regex = re.compile(pairs)
        pairs = list(filter(regex.search, ticker_pairs()))
    return {pair: _chartdata(client, pair, *args, **kwargs) for pair in pairs}
...
def orderbooks(client, pairs, depth=100):
